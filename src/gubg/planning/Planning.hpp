...
namespace gubg { namespace planning {
    ...
    class Planning
    {
        ...
        public:
            ...
            bool get_first_day(Day &day) const
            {
                Day d;
            ...
            bool get_last_day(Day &day) const
            {
                Day d;
                for (auto &p: dayPlanningsPerWorker_)
                {
            ...
            using SweatPerTask = std::map<Task::Ptr, double>;
            void get_sweat_per_task(SweatPerTask &sweat_per_task, const Worker &worker, const std::set<Day> &days, const size_t wanted_level)
            {
                for (auto dp: dayPlanningsPerWorker_[worker])
                {
                    ...
                    for (auto tp: dp.second.taskParts)
                    {
                        ...
                        if (level >= wanted_level)
                            ...
                            level = 0;
                        sweat_per_task[taskAtLevel_(task, level)] += tp.sweat;
                    }
                }
            }
            void overviewForWorker(std::ostream &os, const Worker &worker, const std::set<Day> &days, const size_t wanted_level)
            {
                if (days.empty())
                {
                    os << "Empty period specified" << std::endl;
                ...
                SweatPerTask sweat_per_task;
                get_sweat_per_task(sweat_per_task, worker, days, wanted_level);

                auto b = days.begin();
                auto e = days.end();
                --e;
                size_t maxTaskNameSize = 0;
                for (auto &t_s: sweat_per_task)
                {
